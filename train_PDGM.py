...
def inference(model, hyper, labels, logger, save_infrence=False, test_mode=False ):
   model.eval()
   metric_list = 0.0
   ...
   test_dataset = PDGM_dataset(base_dir=dataset_path, mode="test",  img_size=img_size,shuffle=False ,data_list=hyper['test_list'], test_mode=test_mode)
   ...
   for sample in test_loader:
      ...
      logger.info(f'\t\t    {case_name}:  {metric_mean[0]:.5f}  {metric_mean[1]:.5f}   {metric_mean[2]:.5f}' )

   metric_list /= length
   metric_list[:,:2] = metric_list[:,:2]*100
   mean_metric = np.mean(metric_list,axis=0)
   for i, target in enumerate(labels):
            logger.info(f'\t\t {target} : mIoU {metric_list[i][0]:.3f}, dice {metric_list[i][1]:.3f}, hd95 {metric_list[i][2]:.3f}')

   logger.info(f'\t\t mean mIoU {mean_metric[0]:.3f},mean dice {mean_metric[1]:.3f}, mean hd95 {mean_metric[2]:.3f}')

   return np.asarray(metric_list) #[[miou][dice][hd95]]

def train_epoch(model, criterion, optimizer,scheduler, hyper, logger, test_mode=False, train_loader=None):
    model.train()
    running_loss = 0.0
    running_ce = 0.0
    running_dice = 0.0
    ...
    if train_loader is None:
      ...
      #  train_path = os.path.join(dataset_path,'train_npz')
       train_dataset = PDGM_dataset(base_dir=dataset_path, mode="train",img_size=img_size,data_list=hyper['train_list'],
    ...
    logger.info(f'\t\t loss:{running_loss/length:.4f}')
    logger.info(f'\t\t ce loss   :{running_ce/length:.4f}')
    logger.info(f'\t\t dice loss :{running_dice/length:.4f}')
    logger.info(f'\t\t steps:{length}')
    logger.info(f'\t\t throughput:{throughput:.2f} ')
    logger.info(f'\t\t tps :{batch_size/throughput:.2f}')
    logger.info(f'\t\t lr :{lr_}')
    # return running_loss/length, running_ce/length, running_dice/length, lr_


def _save_best(model,optimizer,hyper,save_epoch_path,epoch, best_metric, labels, logger):
   params = {
     'epoch' : epoch,
   ...
   logger.info(f'\t save best epoch success!')


def train(model, criterion, optimizer,scheduler, hyper,labels, logger, continue_train=False, epoch=None):
   num_classes = hyper['num_classes']
   ...
   if continue_train:
     if os.path.exists(save_epoch_path):
       ...
       logger.info(f'continue training from best epoch {epoch_start}')
       ...
       del params
        ...
        epoch_start = params['epoch'] + 1
        logger.info(f'continue training from epoch {epoch_start}')
        model.load_state_dict(params['state_dict'])
        optimizer.load_state_dict(params['optimizer_state_dict'])
        del params
   ...
   for epoch in range(epoch_start, epoch_num):
       ...
       train_epoch(model=model, criterion=criterion, optimizer=optimizer,scheduler=scheduler,
                   hyper=hyper,test_mode=False, logger=logger)
       ...
       if do_inference:
         ...
         metric = inference(model=model, hyper=hyper,labels=labels[1:],
                            save_infrence=False, test_mode=False, logger=logger)
         condition = np.mean(metric[:,0] + metric[:,1]) >= np.mean(best_metric[:,0] + best_metric[:,1])
         if condition:
           best_metric = metric
           _save_best(model,optimizer,hyper,save_epoch_path,epoch,best_metric,labels[1:], logger=logger)
       logger.info(f'\t time: {(time.time() - t_tick):.2f} s')
...
def train_model(args,hyper=None):  
   ...
   logger = create_logger(output_dir=save_path)
   
   logger.info('PDGM Trainer Initailizing...')
   logger.info(f'result will be saved on {save_path}')
   save_epoch_path = os.path.join(save_path, 'best_epoch.pth')
   
   if os.path.exists(save_epoch_path) or os.path.exists(os.path.join(save_path,'epoch.pth')):
      logger.info("========================warning====================")
      logger.info("This config was trained before, continue training will cover the saved data!")
      time.sleep(3)
      logger.info("====================================================")
     # date = time.strftime("%Y%m%d_%H%M",time.localtime(time.time()))
   ...
   logger.info('PDGM Trainer initalied !')
   if args.continue_train is not False:
      train(model=model,criterion=criterion,optimizer=optimizer,scheduler=scheduler,hyper=hyper,labels=labels,logger=logger,continue_train=True)
   
   if args.continue_train is False and args.test is False: # training from scratch
      train(model,criterion,optimizer,scheduler,hyper,labels, logger=logger, continue_train=False)
   ...
   logger.info(f'plot seg map')
   inference(model=model, hyper=hyper,labels=labels[1:],save_infrence=True, logger=logger)
...
def n_fold_config(config,args):
   ...
   with open(os.path.join(dataset_dir,'dataset.json')) as fp:
      meta = json.load(fp)
      data_list = np.array(meta['data_list'])
   ...
   if n == 1:
      train_list = data_list[:int(data_size*0.8)]
      test_list = data_list[int(data_size*0.8):]
      cfg = copy.deepcopy(config)
      cfg.update({'train_list':train_list,
                  'test_list':test_list})
      with open(os.path.join(save_dir,'config.json'), 'w', encoding='utf-8') as f:
         json.dump(cfg, f, ensure_ascii=False, indent=4,default=lambda cls : cls.__name__ if hasattr(cls,'__name__') else str(cls))
         
      config_list.append(cfg)
   else:
      kf = KFold(n)
      for idx, (train, test) in enumerate(kf.split(data_list)):
         save_fold_dir = os.path.join(save_dir,f'fold{idx + 1}')

         os.makedirs(save_fold_dir,exist_ok=True)
         cfg = copy.deepcopy(config)
         cfg.update({'train_list':data_list[train],
                     'test_list':data_list[test],
                     'save_path':save_fold_dir
                     })
...
if __name__ == '__main__':
   ...
   parser.add_argument('--continue_train', action='store_true',
                       help='continue train from last epoch')
   parser.add_argument('--test', action='store_true', help='evaluate model on test dataset')
   parser.add_argument('--seed', type=int,
                       default=1234, help='random seed')
   parser.add_argument('--n-fold', type=int,
                       default=5, help='n fold corss validation')
   ...
   config_list = n_fold_config(config_PDGM,args)
